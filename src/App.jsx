...
export default function App() {
  ...
  useEffect(() => {
    document.title = `Commercial Analyzer — ${TABS.find((t) => t.id === activeTab)?.label || ''}`
  }, [activeTab])
  ...
  return (
    <div className="page">
      <header className="no-print">
        <h1>REI Commercial Analyzer</h1>
        <p className="sub">Retail, office, warehouse deal analysis · Reads live Math Bible + calls external tools · v{VERSION}</p>
      </header>
      ...
      <footer>
        ...
        <div>REI Commercial Analyzer v{VERSION} · Released {BUILD_DATE}</div>
